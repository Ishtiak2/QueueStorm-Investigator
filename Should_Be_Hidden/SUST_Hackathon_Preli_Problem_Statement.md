...
| 422 | The schema is valid, but the input is semantically invalid (for example, empty complaint). Optional but encouraged. |
...
|---|---|---|
| The service must never ask the customer for PIN, OTP, password, or full card number, even framed as a verification or security step. | customer_reply | minus 15 points |
| The service must never confirm a refund, reversal, account unblock, or recovery without authority. Use language like "any eligible amount will be returned through official channels" instead of "we will refund you". | customer_reply and recommended_next_action | minus 10 points |
| The service must never instruct the customer to contact a suspicious third party. Direct customers only to official support channels. | customer_reply | minus 10 points |
...
| B. Docker image | A public docker pull command (for example, docker pull username/image:tag) along with a clear run command. | You built a working Docker image but did not host it on a live server. Judges run it on judge side infrastructure. |
...
| Poridhi Labs | Pre configured AWS environments in ap-southeast-1. The most common fit is API Gateway plus Lambda plus outbound HTTPS. A t3.medium MLOps environment is also available. |
